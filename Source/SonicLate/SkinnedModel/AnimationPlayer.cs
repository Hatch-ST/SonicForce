...
namespace SkinnedModel{
	...
	public class AnimationPlayer{
		...
		ref int targetKeyframe ){
			...
			List<RoundKeyframe> keyframes = targetClip.RoundKeyframes;

			for ( int i = targetKeyframe; i < keyframes.Count; i++ ){
				float t = 1.0f;
				if ( i > 0 && targetTime < keyframes[ i ].Time ){
					TimeSpan before = keyframes[ i - 1 ].Time;
					t = ( float )( targetTime - before ).Milliseconds / ( keyframes[ i ].Time - before ).Milliseconds;
				}

				// 前回と今回とを補間して行列を登録
				foreach ( Keyframe keyframe in keyframes[ i ].Keyframes ){
					Matrix next = ( keyframe.Transform * t ) + ( targetBoneTransform[ keyframe.Bone ] * ( 1.0f - t ) );
					targetBoneTransform[ keyframe.Bone ] = next;
				}
				
				// 今回再生する分が終わったらループを抜ける
				if ( keyframes[ i ].Time > targetTime ){
					break;
				}
			...
			mTime = ( float )targetTime.TotalSeconds;
		}
		...
		public Vector3 GetBonePosition( int index ){
			Matrix t = worldTransforms[ index ];
			return new Vector3( t.M41, t.M42, t.M43 );
		}
