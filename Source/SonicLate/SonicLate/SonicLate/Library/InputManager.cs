...
namespace SonicLate {
	...
	public class InputManager {
		...
		public static bool IsAnyKeyDown(){
			Keys[] currentKeys = currentKeyState.GetPressedKeys();
			for ( int i = 0; i < currentKeys.Length; i++ ){
				if ( IsJustKeyDown( currentKeys[ i ] ) ){
					return true;
				}
			}
		...
		/// <summary>
		/// 現在のフレームで何らかのボタンが押されたか?
		/// </summary>
		/// <param name="player">プレイヤーインデックス</param>
		/// <returns>true:押されている/false:押されていない</returns>
		public static bool IsJustAnyButtonDown( PlayerIndex player ){
			if ( IsJustButtonDown( player, Buttons.A ) ) return true;
			if ( IsJustButtonDown( player, Buttons.B ) ) return true;
			if ( IsJustButtonDown( player, Buttons.X ) ) return true;
			if ( IsJustButtonDown( player, Buttons.Y ) ) return true;
			if ( IsJustButtonDown( player, Buttons.Back ) ) return true;
			if ( IsJustButtonDown( player, Buttons.Start ) ) return true;
			if ( IsJustButtonDown( player, Buttons.BigButton ) ) return true;
			if ( IsJustButtonDown( player, Buttons.DPadDown ) ) return true;
			if ( IsJustButtonDown( player, Buttons.DPadLeft ) ) return true;
			if ( IsJustButtonDown( player, Buttons.DPadRight ) ) return true;
			if ( IsJustButtonDown( player, Buttons.DPadUp ) ) return true;
			if ( IsJustButtonDown( player, Buttons.LeftShoulder ) ) return true;
			if ( IsJustButtonDown( player, Buttons.LeftTrigger ) ) return true;
			if ( IsJustButtonDown( player, Buttons.RightShoulder ) ) return true;
			if ( IsJustButtonDown( player, Buttons.RightTrigger ) ) return true;
			return false;
		}
